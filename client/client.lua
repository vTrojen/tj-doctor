                ...
                if userDst <= 5 then
                    ...
                    if userDst <= 1.5 then
                        if IsControlJustPressed(0, 38) then
                            ESX.TriggerServerCallback('trojen:getEms', function(cb)
                                canEms = cb
                            end, i)
                            while canEms == nil do
                                Wait(0)
                            ...
                            if Config.DoctorLimit then
                                if canEms == true then
                                    ...
                                    ESX.TriggerServerCallback('trojen-npcdoctor:parakontrol', function(hasEnoughMoney)
                                        if hasEnoughMoney then
                                            ...
                                            TriggerServerEvent('trojen-npcdoctor:money')
                                        else
                                ...
                                elseif canEms == 'no_ems' then
                                    exports['mythic_notify']:SendAlert('error', 'Şehirde yeterli Doktor var onlara tedavi ol')                          
                                end 
                            else
                                ESX.TriggerServerCallback('trojen-npcdoctor:parakontrol', function(hasEnoughMoney)
                                    if hasEnoughMoney then
